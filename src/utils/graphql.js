...
function __request(
  ...
  if (asJson) {
    ...
    var body =
      ...
      "&variables=" +
      encodeURIComponent(JSON.stringify(data.variables));
  }
  if (debug) {
...
function __isTagCall(strings) {
  return (
    Object.prototype.toString.call(strings) == "[object Array]" && strings.raw
  );
}
  ...
  if (!(this instanceof GraphQLClient)) {
    ...
    for (var m in client) {
      if (typeof client[m] == "function") {
        ...
        if (client[m].declare)
          _lazy[m].declare = client[m].declare.bind(client);
    ...
    throw new Error(
      "You cannot create GraphQLClient instance. Please call GraphQLClient as function."
    );
  }
  if (!options) options = {};
...
GraphQLClient.prototype.fragmentPath = function (fragments, path) {
  var getter = new Function(
    "fragments",
    "return fragments." + path.replace(/\./g, FRAGMENT_SEPERATOR)
  );
  ...
  (query.match(fragmentRegexp) || []).forEach(function (fragment) {
    ...
    if (fragment) {
      var pathRegexp = new RegExp(
        fragmentRegexp.source.replace(/\((.*)\)/, path) + "$"
      );
      ...
      var alreadyCollectedFragments = collectedFragments.filter(function (
        alreadyCollected
      ) {
        return alreadyCollected.match(new RegExp("fragment " + path));
      });
      if (
        alreadyCollectedFragments.length > 0 &&
        fragmentRegexp.test(fragment)
      ) {
...
GraphQLClient.prototype.processQuery = function (query, fragments) {
  if (
    typeof query == "object" &&
    query.hasOwnProperty("kind") &&
    query.hasOwnProperty("definitions")
  ) {
    throw new Error(
      "Do not use graphql AST to send requests. Please generate query as string first using `graphql.print(query)`"
    );
  }
  var fragmentRegexp = GraphQLClient.FRAGMENT_PATTERN;
  ...
  return [query]
    .concat(
      collectedFragments.filter(function (fragment) {
    ...
    caller.merge = function (mergeName, variables) {
      if (!type) {
        ...
        query = query
          ...
          .trim()
          .replace(GraphQLClient.AUTODECLARE_PATTERN, "")
          .trim()
          .replace(/^\{|\}$/g, "");
      }
      if (!originalQuery) {
...
GraphQLClient.prototype.commit = function (mergeName) {
  if (!this._transaction[mergeName]) {
    throw new Error(
      "You cannot commit the merge " + mergeName + " without creating it first."
    );
  }
  var that = this;
  var resolveMap = {};
  ...
  Object.keys(this._transaction[mergeName]).forEach(function (method) {
    ...
    var subQuery = that._transaction[mergeName][method]
      .map(function (merge) {
        var reqId =
          "merge" + Math.random().toString().split(".")[1].substr(0, 6);
        resolveMap[reqId] = merge.resolver;
        var query = merge.query.replace(/\$([^\.\,\s\)]*)/g, function (_, m) {
          if (!merge.variables) {
            throw new Error(
              "Unused variable on merge " + mergeName + ": $" + m[0]
            );
          }
    ...
    mergedQueries[method].push(
      method + " (@autodeclare) {\n" + subQuery + "\n }"
    );
  });

  return Promise.all(
    Object.keys(mergedQueries).map(function (method) {
    ...
    .then(function (responses) {
      ...
      responses.forEach(function (response) {
        Object.keys(response).forEach(function (mergeKey) {
          ...
          if (!parsedKey) {
            throw new Error(
              "Multiple root keys detected on response. Merging doesn't support it yet."
            );
          }
          ...
          newResponses[fieldName] = (newResponses[fieldName] || []).concat([
            response[mergeKey],
          ]);
...
GraphQLClient.prototype.createHelpers = function (sender) {
  ...
  helpers.forEach(function (m) {
    that[m.method] = function (query, variables, options) {
      if (
        that.options.alwaysAutodeclare === true ||
        (options && options.declare === true)
      ) {
        return helper.call(
          { type: m.type, prefix: m.type + " (@autodeclare) {", suffix: "}" },
          query,
          variables
        );
      }
      return helper.call(
        { type: m.type, prefix: m.type, suffix: "" },
        query,
        variables
      );
    };
...
GraphQLClient.prototype.fragment = function (fragment) {
  if (typeof fragment == "string") {
    var _fragment = this._fragments[
      fragment.replace(/\./g, FRAGMENT_SEPERATOR)
    ];
    if (!_fragment) {
...
GraphQLClient.prototype.ql = function (strings) {
  ...
  var query =
    typeof strings == "string"
      ? strings
      : strings.reduce(function (acc, seg, i) {
